...
export default function DeckPlansPage() {
  ...
  const query = `${maxPrice}|${targetBracket ?? ""}|${forced.join(",")}`

  useEffect(() => {
    deckPlansApi
      .build(maxPrice, targetBracket, forced)
      .then((result) => {
        setAnswer({ query: `${maxPrice}|${targetBracket ?? ""}|${forced.join(",")}`, result })
        setError(null)
      })
      .catch((err) => setError(err instanceof Error ? err.message : "Erreur inattendue"))
  }, [maxPrice, targetBracket, forced])
            ...
            <>
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                {selection.plans.map((plan) => (
                  <DeckPlanCard key={plan.commander.oracle_id} plan={plan} coreSize={data.core_size} />
                ))}
              </div>
...
function CommanderRow({
  ...
  return (
    <tr className={`border-b last:border-0 ${retained ? "" : "text-muted-foreground"}`}>
      ...
      <td className="py-1.5 text-right tabular-nums">{plan.to_buy_count}</td>
      <td className="py-1.5 text-right tabular-nums">{plan.cost_eur.toFixed(2)} €</td>
...
function DeckPlanCard({ plan, coreSize }: { plan: DeckPlan; coreSize: number }) {
      ...
      <CardContent className="flex flex-col gap-3 text-sm">
        ...
        <div className="flex flex-col gap-1">
          ...
          <span className="text-muted-foreground">
            {plan.lands.owned_nonbasic.length > 0 &&
              `${plan.lands.owned_nonbasic.length} terrain(s) non-basique(s) déjà possédé(s) · `}
            {basics.map(([name, count]) => `${count} ${name}`).join(" · ")}
          </span>
        </div>
